...
class TestCLI:
    ...
    def test_docs_openapi_get_openapi_called(self, mocker: MockerFixture, runner: CliRunner) -> None:
        """Make sure the 'get_openapi' function is called."""
        mock_get_openapi = mocker.patch("app.commands.docs.get_openapi", return_value={})
        result = runner.invoke(app, ["docs", "openapi"])

        assert result.exit_code == 0
        mock_get_openapi.assert_called_once()

    def test_docs_openapi_json_dump_called(self, mocker: MockerFixture, runner: CliRunner) -> None:
