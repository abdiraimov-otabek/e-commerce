...
class TestSecurityManager:
    ...
    async def test_get_current_user_jwt(self, test_db, mocker) -> None:
        ...
        result = await get_current_user(mock_request, jwt_user=user, api_key_user=None)
        assert result == user
    ...
    async def test_get_current_user_api_key(self, test_db, mocker) -> None:
        ...
        mock_request = mocker.Mock()
        result = await get_current_user(mock_request, jwt_user=None, api_key_user=user)
        assert result == user
    ...
    async def test_get_current_user_no_auth(self, test_db, mocker) -> None:
        ...
        with pytest.raises(HTTPException) as exc:
            await get_current_user(mock_request, jwt_user=None, api_key_user=None)

        assert exc.value.status_code == status.HTTP_401_UNAUTHORIZED
        assert exc.value.detail == "Not authenticated. Use either JWT token or API key."
        assert exc.value.headers == {"WWW-Authenticate": "Bearer or ApiKey"}

    async def test_get_optional_user_authenticated(self, test_db, mocker) -> None:
